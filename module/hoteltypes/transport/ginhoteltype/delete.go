...
func DeleteHotelType(appCtx appContext.AppContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		biz := hoteltypebiz.NewDeleteHotelTypeBiz(store)

		if err := biz.DeleteHotelType(c.Request.Context(), int(uid.GetLocalID())); err != nil {
			panic(err)
		}
