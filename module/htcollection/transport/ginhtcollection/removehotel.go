...
func RemoveHotelFromCollection(appCtx appContext.AppContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		biz := htcollectionbiz.NewRemoveHotelBiz(store)
		requester := c.MustGet(common.CurrentUser).(common.Requester)
		if err = biz.RemoveHotelFromCollection(c.Request.Context(),
			int(hotelUid.GetLocalID()),
			int(collectionUid.GetLocalID()),
			requester); err != nil {
			panic(err)
		}
