...
func (s *sqlStore) UpdateReviewWhenDeleteReview(ctx context.Context, review *common.DTOReview) error {
	db := s.db.Table(hotelmodel.Hotel{}.TableName()).Where("id = ?", review.HotelId)
...
func (s *sqlStore) UpdateTotalRoomTypeAndAvgPriceWhenDeleteRoomType(ctx context.Context, room *common.DTORoomType) error {
	db := s.db.Table(hotelmodel.Hotel{}.TableName()).Where("id = ?", room.HotelId)
